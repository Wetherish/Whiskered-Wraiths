        ...
        void Update()
{
    ...
    hpStats.HeroHealth = hpStats.HeroHealth > 2 * hpStats.MaxNumberOfHearts ? 2 * hpStats.MaxNumberOfHearts : hpStats.HeroHealth;

    for(int heartIteraor = 0; heartIteraor < hearts.Length; heartIteraor++)
    {
        hearts[heartIteraor].sprite = hpStats.HeroHealth%2 != 0 && heartIteraor == hpStats.HeroHealth/2 ? halfHearts : 
                                       heartIteraor < hpStats.HeroHealth/2 ? fullHearts : emptyHearts;
